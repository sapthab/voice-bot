...
-- Conversations policies (allow public read for widget)
CREATE POLICY "Anyone can view conversations by visitor_id" ON conversations FOR SELECT USING (true);
CREATE POLICY "Anyone can create conversations" ON conversations FOR INSERT WITH CHECK (true);
CREATE POLICY "Members can view all agent conversations" ON conversations FOR SELECT
  USING (agent_id IN (SELECT id FROM agents WHERE organization_id IN (SELECT organization_id FROM organization_members WHERE user_id = auth.uid())));
...
-- Function to increment chat credits
